...
export default function Slots() {
  ...
  const loadData = async () => {
    if (!date) { showSnack("Please select a date", "warning"); return; }
    try {
      const bkRes = await fetch(`${import.meta.env.VITE_API_URL}/api/bookings/all`);
      const bkData = await bkRes.json();
      setBookings(bkData.data || []);

      const slRes = await fetch(`${import.meta.env.VITE_API_URL}/api/slots?date=${date}`);
  ...
  const isBooked = (time) =>
    bookings.some((b) => b.date === date && b.time === time && b.status === "confirmed");
  ...
  const getFinalStatus = (time) => {
    ...
    if (booking && (booking.status === "cancelled" || booking.status === "no-show")) return "available";
  ...
  const applyAction = () => {
    ...
    setSlots((prev) => {
      ...
      selectedSlots.forEach((time) => {
        ...
        if (!exists) { updated.push({ time, status: action }); }
        else { updated = updated.map((s) => s.time === time ? { ...s, status: action } : s); }
      });
      return updated;
    });
    setSelectedSlots([]);
  ...
  const saveSlots = async () => {
    ...
    } catch { showSnack("Error saving slots", "error"); }
  };
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2">
        {TIMES.map((time) => {
          ...
          let cls = "bg-emerald-50 border-emerald-200 text-emerald-700 hover:bg-emerald-100"; // available
          if (booked) cls = "bg-red-50 border-red-200 text-red-500 cursor-not-allowed opacity-75";
          if (isSelected) cls = "bg-amber-50 border-amber-300 text-amber-700 ring-2 ring-amber-300";
      ...
      {selectedSlots.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 space-y-3">
          <p className="text-slate-600 text-sm font-medium">{selectedSlots.join(", ")}</p>
      ...
      {/* Toast */}
      {snack.open && (
        <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 px-5 py-3 rounded-2xl text-sm font-medium shadow-xl
